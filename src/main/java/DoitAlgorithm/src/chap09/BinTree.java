...
public class BinTree<K, V> {
    ...
    public BinTree(Comparator<? super K> c) { // 비교자로 키값을 비교
        this();
        comparator = c;
    }
    ...
    public boolean remove(K key) {
        ...
        if (p.left == null) { // p에 왼쪽 자식이 없음
            ...
            isLeftChild = true;
            while (left.right != null) { // 가장 큰 노들 left를 검색
            }
    ...
    private void printReverseSubTree(Node node) {
        if (node != null) {
            printSubTree(node.right); // 오른쪽 서브트리를 키값의 내림차순으로 출력
            System.out.println(node.key + " " + node.data); // node를 출력
            printSubTree(node.left); // 왼쪽 서브트리를 키값의 내림차순으로 출력
        }
    }
    public void printReverse() { // 키값의 내림차순으로 모든 노드를 출력
    ...
    // 가장 작은 키값을 갖는 데이터를 반환
    V getDataWithMinKey() {
        return search(getMinKey());
    }

    // 가장 큰 키값을 갖는 노드의 데이터를 반환
    V getDataWithMaxKey() {
